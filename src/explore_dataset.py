"""
GTZAN Dataset Exploration Script
Analyse complète du dataset pour identifier les problèmes et statistiques
"""
...
# ============================================================================
# 6. ANALYSE AUDIO APPROFONDIE (Échantillon)
# ============================================================================
print("\n" + "=" * 70)
print("🎵 6. ANALYSE AUDIO APPROFONDIE (5 fichiers par genre)")
print("=" * 70)
...
# ============================================================================
# 7. VISUALISATION DES SPECTROGRAMMES (1 exemple par genre)
# ============================================================================
print("\n" + "=" * 70)
...
print(f"""
✅ Dataset GTZAN chargé avec succès!

📊 Résumé:
   - Total de fichiers: {len(df)}/1000
   - Fichiers corrompus: {len(corrupted_files)}
   - Durée moyenne: {df['duration'].mean():.2f}s
   - Sample rate principal: {df['sample_rate'].mode()[0]} Hz
   - Taille totale: {df['file_size_mb'].sum():.2f} MB
   
🎭 Genres équilibrés: {'✅ Oui' if is_balanced else '❌ Non'}

⚠️  Points d'attention:
   {'- Fichiers corrompus détectés' if corrupted_files else '- Aucun fichier corrompu'}
   {'- Sample rates hétérogènes (resampling nécessaire)' if len(sample_rates) > 1 else '- Sample rates homogènes'}
   {'- Durées anormales détectées' if len(abnormal_duration) > 0 else '- Toutes les durées sont normales'}

🎯 Prochaines étapes:
   1. Créer les splits train/val/test (70/15/15)
   2. Exclure les fichiers corrompus
   3. Resampler à 16kHz pour WAV2VEC
   4. Extraire les features MFCC pour baseline
""")
